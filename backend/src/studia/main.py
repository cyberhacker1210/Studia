...
app = FastAPI(title="Studia API", version="2.4.0")
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Autorise toutes les origines (localhost, vercel, etc.)
...
# --- Modèles pour le Parcours Adaptatif ---
...
async def extract_text_endpoint(request: ExtractTextRequest):
    try:
        ...
        def get_base64(uri):
            return uri.split("base64,")[1] if "base64," in uri else uri
...
async def generate_quiz_text(request: QuizGenerateFromTextRequest):
    try:
        ...
        # Normalisation des questions pour le frontend
        questions = []
        for i, q in enumerate(quiz_data.get("questions", [])):
            # Gestion de correct_index (nouveau) vs correctAnswer (ancien)
            correct = q.get("correctAnswer") if "correctAnswer" in q else q.get("correct_index", 0)
        ...
        return QuizResponse(id=str(uuid.uuid4()), questions=questions, createdAt=datetime.now().isoformat(),
                            extractedText=request.course_text)
    except Exception as e:
        print(f"❌ Error Quiz Text: {e}")
...
async def generate_flashcards_endpoint(request: FlashcardGenerateRequest):
    try:
        ...
        cards = [Flashcard(front=c.get("front"), back=c.get("back"), category=c.get("category", "Général")) for c in
                 data.get("flashcards", [])]
...
# --- 4. PARCOURS ADAPTATIF (NOUVEAU SYSTEME) ---

@app.post("/api/path/diagnostic")
async def diagnostic_endpoint(req: CourseRequest):
    try:
        return generate_diagnostic_quiz(req.course_text)
    except Exception as e:
        print(f"❌ Error Diagnostic: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def remediation_endpoint(req: RemediationRequest):
    try:
        return generate_remediation_content(req.course_text, req.weak_concepts, req.difficulty)
    except Exception as e:
        print(f"❌ Error Remediation: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def validation_endpoint(req: ValidationRequest):
    try:
        return generate_validation_quiz(req.course_text, req.concepts, req.difficulty)
    except Exception as e:
...
async def practice_endpoint(req: PracticeRequest):
    try:
        return generate_practice_exercise(req.course_text, req.difficulty)
    except Exception as e:
...
async def evaluate_answer_endpoint(req: EvalRequest):
    try:
        return evaluate_student_answer(req.instruction, req.student_answer, req.course_context)
    except Exception as e:
